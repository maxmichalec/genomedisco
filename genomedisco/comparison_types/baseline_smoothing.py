...
def analysis(args, m1, m2, sm1, sm2, final_diff_vector, reproducibility, name):
    #now, make 1 plot
    if not args.concise_analysis:
        originals=np.triu(m1.toarray())-np.triu(m2.toarray()).T
        rw=np.triu(sm1.toarray())-np.triu(sm2.toarray()).T
        diff_mat=(sm1-sm2).toarray()
    ...
    if not args.concise_analysis:
        ...
        for t in range(args.tmin,args.tmax+1):
            ...
            fname=args.outdir+'/'+args.outpref+'.'+args.m1name+'.vs.'+args.m2name+'.'+name+'.'+str(t)+'.png'
    ...
    return reproducibility_text, reproducibility_text_rw
...
class Baseline:
    ...
    def compute_reproducibility(self,m1_csr,m2_csr,args):
        ...
        diff_vector+=abs(m1-m2).sum(axis=1)
        diff=abs(m1-m2).sum()#+euclidean(rw1.toarray().flatten(),rw2.toarray().flatten()))
        scores.append(1.0*float(diff)/float(nonzero_total))
        extra_text=' | score='+str('{:.3f}'.format(1.0-float(diff)/float(nonzero_total)))
        print 'GenomeDISCO | '+strftime("%c")+' | done'+extra_text
        ...
        reproducibility_text, reproducibility_text_rw = analysis(args, m1, m2, m1, m2, final_diff_vector, reproducibility, 'Baseline')
...
class FixedWindow:
    ...
    def compute_reproducibility(self,m1_csr,m2_csr,args):
        ...
        if True:
            ...
            for t in range(1, args.tmax+1): #range(args.tmin,args.tmax+1):
                ...
                if t % 2 == 0:
                    continue
                if t == 1:
                    dm1=copy.deepcopy(m1).toarray()
                    dm2=copy.deepcopy(m2).toarray()
                    if args.transition:
                        m1=to_transition(m1)
                        m2=to_transition(m2)
                else:
                    k = np.ones((t, t)) / (t * t)  # smoothing kernel
                    sm1 = convolve2d(dm1, k, mode='same', boundary='symm')
                    sm2 = convolve2d(dm2, k, mode='same', boundary='symm')
                    sm1 = csr_matrix(sm1)
                    sm2 = csr_matrix(sm2)
                if t != 1:
                    if args.transition:
                        sm1=to_transition(sm1)
                        sm2=to_transition(sm2)
                    diff_vector+=abs(sm1-sm2).sum(axis=1)
                    diff=abs(sm1-sm2).sum()#+euclidean(rw1.toarray().flatten(),rw2.toarray().flatten()))
                    scores.append(1.0*float(diff)/float(nonzero_total))
                    extra_text=' | score='+str('{:.3f}'.format(1.0-float(diff)/float(nonzero_total)))
                print 'GenomeDISCO | '+strftime("%c")+' | done w='+str(t)+extra_text
        ...
        if args.tmin==args.tmax:
            ...
            auc=metrics.auc(range(len(scores)),scores)/denom
        reproducibility=1.0-auc
        ...
        reproducibility_text, reproducibility_text_rw = analysis(args, m1, m2, sm1, sm2, final_diff_vector, reproducibility, 'FixedWindow')
...
class GaussianSmoothing:
    ...
    def compute_reproducibility(self,m1_csr,m2_csr,args):
        ...
        if True:
            ...
            for s in sigmas:
                ...
                if s == 0:
                    ...
                    sm1 = gaussian_filter(dm1, sigma=s)
                    sm2 = gaussian_filter(dm2, sigma=s)
                    sm1 = csr_matrix(sm1)
                    sm2 = csr_matrix(sm2)
                if s != 0:
                    if args.transition:
                    ...
                    scores.append(1.0*float(diff)/float(nonzero_total))
                    extra_text=' | score='+str('{:.3f}'.format(1.0-float(diff)/float(nonzero_total)))
                print 'GenomeDISCO | '+strftime("%c")+' | done sigma='+str(s)+extra_text
        ...
        if args.tmin==args.tmax:
            ...
            auc=metrics.auc(range(len(sigs)),scores)/denom
        reproducibility=1.0-auc
        ...
        reproducibility_text, reproducibility_text_rw = analysis(args, m1, m2, sm1, sm2, final_diff_vector, reproducibility, 'GaussianSmoothing')
